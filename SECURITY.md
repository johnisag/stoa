...
| `.github/workflows/*`, composite `actions/*`                                                                                                                                        | every CI run                                                                                                                                                                                       | content-pinned; CI `permissions: contents: read`; actions SHA-pinned; **the CI guard runs from the BASE-branch copy**, so a PR can't neuter its own check                                                                                                    |
| the guard's own config + pins (`security/`)                                                                                                                                         | —                                                                                                                                                                                                  | byte-pinned **and** code-owned; `guard.config.json` is **fail-closed** — it can only WIDEN coverage / TIGHTEN limits, never disarm the guard                                                                                                                 |
| `.claude/*`, `.cursor/*`, `.gemini/*`, `.claude.json`, `.mcp.json` `hooks` + **MCP servers**                                                                                        | every Claude/Cursor/Gemini/Codex/Hermes start (Cursor hooks fire on `workspaceOpen` with **no approval**; Gemini folder-trust is **OFF by default**, so a committed `.gemini/` auto-execs ungated) | structured `*.json` scan: fails on any `hooks` key or any non-allowlisted MCP server (shells/metachars/decoys rejected); an _unparseable_ config is itself a violation — **on top of** the byte-pin above                                                    |
...
**When you legitimately change a surface**, re-pin in the SAME PR. Because
`security/` is code-owned (`.github/CODEOWNERS`), re-pinning is a reviewed change —
the trust root can't be silently re-baselined onto a trojan:
...
default since they double as hand-edited docs); absent ⇒ baked defaults. Overrides are **fail-closed** — they
can only widen coverage / tighten limits (coverage lists are unioned with the
defaults, `maxFileBytes` can only drop), so a committed config can't disarm the guard.
