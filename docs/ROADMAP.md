...
- [ ] **Unlock codex/hermes orchestration workers** — _(P:high · E:M)_ The conductor/worker path is plumbed for `agentType` end-to-end, but the MCP `spawn_worker` tool lacks an `agentType` param (model defaults to `sonnet`) and `spawnWorker`'s readiness loop hard-matches Claude-only banners — so an LLM conductor can only spawn Claude workers, and non-Claude workers burn the 30s timeout then blind-send. Add `agentType` to the tool schema + a per-provider readiness/trust-prompt contract via `getProvider` (Claude byte-identical). Unlocks the heterogeneous-orchestration differentiator that's ~90% built. _Where:_ mcp/orchestration-server.ts, lib/orchestration.ts:196-265, lib/providers/registry.ts (the vestigial `waitingPatterns` finally get a home). _Risk:_ no tmux/lifecycle/status-detector touch.
