...
- [~] **Wire pane/tab/view keyboard shortcuts** — _partial_ (PR #23): `⌘/Ctrl+B` toggle sidebar + `⌘/Ctrl+\` split the focused pane (browser/terminal-safe chords, auto-listed in the cheatsheet). **Remaining:** tab nav + view toggles (git/files/shell/terminal) — those need the pane-local `viewMode`/`rightDrawer` lifted (or a focused-pane request store like `fileOpenStore`) and conflict-safe chords (⌘W/⌘T are browser-reserved; Opt+arrows are terminal word-nav). _Original:_ all multi-pane layout ops (split, close, focus-next, new/switch tab, git/files/shell toggles) are mouse-only icon buttons though `PaneContext` already exposes the methods. Bind a focused set (`mod+\`, `mod+w`, `mod+t`, `mod+1..9`, `mod+alt+arrows`, view toggles) into `useGlobalKeybindings`; lift pane-local `viewMode` so a global handler can reach the focused pane. _(high/M, Panes; see contexts/PaneContext.tsx, components/Pane/DesktopTabBar.tsx, components/Pane/index.tsx)_
