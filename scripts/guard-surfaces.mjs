...
const DANGEROUS_ENV =
  /^(NODE_OPTIONS|NODE_REPL_EXTERNAL_MODULE|NODE_PATH|BUN_INSPECT|DENO_|LD_PRELOAD|LD_AUDIT|LD_LIBRARY_PATH|DYLD_INSERT_LIBRARIES|DYLD_FRAMEWORK_PATH|DYLD_LIBRARY_PATH|PYTHONSTARTUP|PYTHONPATH|PERL5OPT|RUBYOPT|RUBYLIB)$/i;
...
export function findMcpServers(obj) {
  ...
  for (const mapKey of ["mcpServers", "mcp_servers"]) {
    ...
    for (const [name, def] of Object.entries(map)) {
      ...
      const command =
        isObj && typeof def.command === "string" ? def.command : "";
 ...
 *     are all rejected; only `npx tsx <path>/orchestration-server.ts` matches.
 * This blocks substring decoys, typosquats, comment-smuggling, shell/preload
 * wrappers, directory-name spoofing, and bare-name confusion. NOTE: a name-based
 * allowlist can't fully authenticate an UNPINNED command (an attacker can name a
...
export function isAllowedMcpServer({ command, args, env } = {}, allowlist) {
  ...
  for (const [k, v] of Object.entries(
    ...
    if (
      typeof v === "string" &&
      (META_RE.test(v) || CODE_LOAD_FLAG.test(` ${v} `))
    )
      return false;
  }
...
export function findHooksKeys(obj, path = "") {
  ...
  for (const [key, val] of Object.entries(obj)) {
    ...
    if (val && typeof val === "object")
      found.push(...findHooksKeys(val, `${path}${key}.`));
  }
  return found;
}

/** Obfuscation signal: a very long single line OR a high AVERAGE line length. */
export function isLikelyMinified(
  content,
  maxLineLength = 5000,
  maxAvgLineLength = 1000
) {
...
export function checkPackageScripts(pkg, pinnedScripts) {
  ...
  for (const [name, cmd] of Object.entries(live)) {
    if (!(name in pinned))
      violations.push(`package.json: new script "${name}": ${cmd}`);
    else if (pinned[name] !== cmd)
      violations.push(
        `package.json: script "${name}" changed -> "${cmd}" (pinned: "${pinned[name]}")`
      );
  }
  for (const name of Object.keys(pinned))
    if (!(name in live))
      violations.push(`package.json: script "${name}" removed`);
  return violations;
}
...
export function scriptFileTargets(cmd) {
  ...
  const out = new Set();
  const extRe =
    /(?:^|[\s;&|=("'`])((?:\.{0,2}\/)?[\w./\\-]+\.(?:js|mjs|cjs|ts|mts|cts|sh|bash|zsh|py|rb|ps1|psm1|bat|cmd|pl|php|lua))/gi;
  let m;
  while ((m = extRe.exec(s)) !== null) out.add(norm(m[1]));
  // interpreter + first operand (extensionless shebang scripts, e.g. `node bin/x`).
  const interpRe =
    /(?:^|[\s;&|])(?:node|deno|bun|tsx|ts-node|python3?|ruby|perl|php|sh|bash|zsh|dash)(?:\.exe)?\s+((?:\.{0,2}\/)?[\w./\\-]+)/gi;
  while ((m = interpRe.exec(s)) !== null) {
    const operand = m[1];
    if (!operand.startsWith("-")) out.add(norm(operand)); // skip flags
  }
  return [...out];
}
...
export function discoverSurfaceFiles(relFiles, surfaceDirs, surfaceFiles = []) {
  ...
  const under = (rel) => {
    ...
    return (
      roots.has(lc) || dirsLc.some((d) => lc === d || lc.startsWith(d + "/"))
    );
  };
  return relFiles
    .filter(
      (f) =>
        under(f) && !f.toLowerCase().startsWith(".husky/_/") && f !== PINS_PATH
    )
    .sort();
}
...
export function loadConfig(root, { tracked } = {}) {
  ...
  const untrusted =
    exists && tracked instanceof Set && !tracked.has(CONFIG_PATH);
  ...
  cfg.surfaceFiles = uniq([
    ...DEFAULTS.surfaceFiles,
    ...asArr(raw.surfaceFiles),
  ]);
  ...
  cfg.agentConfigDirs = uniq([
    ...DEFAULTS.agentConfigDirs,
    ...asArr(raw.agentConfigDirs),
  ]);
  cfg.agentConfigFiles = uniq([
    ...DEFAULTS.agentConfigFiles,
    ...asArr(raw.agentConfigFiles),
  ]);
  cfg.localConfigFiles = uniq([
    ...DEFAULTS.localConfigFiles,
    ...asArr(raw.localConfigFiles),
  ]);
  cfg.globalTargets = uniq([
    ...DEFAULTS.globalTargets,
    ...asArr(raw.globalTargets),
  ]);
  cfg.mcpAllowlist = uniq([
    ...DEFAULTS.mcpAllowlist,
    ...asArr(raw.mcpAllowlist).filter(safeAllowEntry),
  ]);
...
function surfaceCandidates(root, cfg, files, skipDirs) {
  ...
  for (const d of cfg.surfaceDirs) {
    ...
    if (existsSync(abs))
      for (const f of walkRel(root, skipDirs, abs)) set.add(f);
  }
...
export function runGuard(root) {
  ...
  const inSubmodule = (rel) =>
    submods.some((p) => rel === p || rel.startsWith(p + "/"));
  const committed = (rel) => tracked.has(rel) || inSubmodule(rel);

  // 0a. An untracked / gitignored guard config is IGNORED and is itself a violation
  ...
  for (const d of cfg.surfaceDirs) {
    try {
      if (lstatSync(join(root, d)).isSymbolicLink())
        violations.push(
          `${d}: surface dir is a symlink (its target isn't scanned/pinned) - not allowed`
        );
    } catch {
      /* absent - fine */
    }
  ...
  for (const p of submods) {
    ...
    if (
      cfg.surfaceDirs.some((d) => {
        const dl = d.toLowerCase();
        return lc === dl || lc.startsWith(dl + "/") || dl.startsWith(lc + "/");
      })
    )
      violations.push(
        `${p}: git submodule at a surface path - its contents can't be pinned and auto-load on a recursive clone; not allowed`
      );
  }
  ...
  if (!pinsParsed.ok) {
    violations.push(
      `${PINS_PATH} missing or unparseable - run 'node scripts/guard-surfaces.mjs --init' (or --update)`
    );
      ...
      for (const name of Object.keys(scripts)) {
        ...
        for (const target of scriptFileTargets(scripts[name])) {
          if (existsSync(join(root, target)) && !(target in pinnedFiles))
            violations.push(
              `package.json: lifecycle script "${name}" runs UNPINNED file "${target}"`
            );
        }
      }
  ...
  for (const rel of surface) {
    ...
    else if (pinnedFiles[rel] !== hash)
      violations.push(`${rel}: content changed from its pinned hash`);
  }
  ...
  for (const rel of new Set([...files, ...surface])) {
    ...
    if (
      cfgFilesLc.has(lc) ||
      (cfgDirsLc.some((d) => lc.startsWith(d + "/")) && lc.endsWith(".json"))
    )
      configRels.add(rel);
  }
  for (const localCfg of cfg.localConfigFiles)
    if (!tracked.has(localCfg) && existsSync(join(root, localCfg)))
      configRels.add(localCfg);
  for (const rel of configRels) {
    ...
    if (hooks.length)
      violations.push(
        `${rel}: contains hook definition(s) [${hooks.join(", ")}] - auto-executes on agent events`
      );
  ...
  if (existsSync(join(root, tasksRel)))
    (tracked.has(tasksRel) ? violations : warnings).push(
      `${tasksRel}: present - auto-runs in the editor`
    );
  ...
  for (const rel of files) {
    ...
    if (size > cfg.maxFileBytes && !oversize.has(basename(rel))) {
      violations.push(
        `${rel}: ${(size / 1e6).toFixed(1)} MB - oversized; possible dropped payload`
      );
      continue;
    }
    if (scriptExts.has(extOf(rel).toLowerCase()) && size <= cfg.maxFileBytes) {
      const content = readText(root, rel);
      if (content != null && isLikelyMinified(content))
        violations.push(`${rel}: minified/obfuscated source`);
    }
  }
...
export function extractGlobalSurfaces(rel, content) {
  if (rel.endsWith(".json")) {
    ...
    if (!p.ok)
      return {
        mcpServers: [],
        hooks: /"hooks"/.test(content),
        parseError: true,
      };
    return {
      mcpServers: findMcpServers(p.value)
        .map((s) => s.name)
        .sort(),
      hooks: findHooksKeys(p.value).length > 0,
    };
  }
  ...
  const names = new Set();
  const head =
    /\[\s*(?:mcp_servers|mcp\.servers|mcpServers)\.(?:"([^"]+)"|([A-Za-z0-9_.-]+))\s*\]/g;
  const dotted =
...
function snapshotGlobal(home, targets) {
  ...
  for (const rel of targets) {
    ...
    snap[rel] = {
      ...
      ...extractGlobalSurfaces(rel, content),
    };
  }
  return snap;
}
...
export function runGlobalGuard(home, targets = DEFAULTS.globalTargets) {
  ...
  for (const rel of targets) {
    ...
    if (!was && now) {
      alerts.push(
        `${rel}: NEW global config since baseline (mcpServers: [${now.mcpServers.join(", ")}], hooks: ${now.hooks})`
      );
      continue;
    }
    if (was && !now) {
    ...
    if (added.length)
      alerts.push(
        `${rel}: NEW MCP server(s) [${added.join(", ")}] - a global auto-launcher was added`
      );
    ...
    if (now.hash !== was.hash && !added.length && !(now.hooks && !was.hooks))
      alerts.push(
        `${rel}: content changed since baseline - review for an injected MCP server / hook`
      );
  }
...
export function writeGlobalBaseline(home, targets = DEFAULTS.globalTargets) {
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        _comment:
          "Stoa global agent-config baseline. Re-baseline: node scripts/guard-surfaces.mjs --global --update",
        files: snap,
      },
      null,
      2
...
function ensurePreCommit(root) {
  ...
  if (existsSync(join(root, ".husky"))) {
    ...
    if (cur.includes("guard-surfaces.mjs"))
      return "pre-commit: already wired (husky)";
  ...
  if (existsSync(join(root, ".git"))) {
    ...
    const cur = existsSync(hook) ? readFileSync(hook, "utf-8") : "";
    if (cur.includes("guard-surfaces.mjs"))
      return "pre-commit: already wired (.git/hooks)";
    const body = cur.startsWith("#!")
      ? cur.replace(/\s*$/, "\n") + line + "\n"
      : `#!/bin/sh\n${cur}${line}\n`;
...
function ensureCiWorkflow(root) {
  ...
  if (existsSync(file))
    return "ci: .github/workflows/surface-guard.yml already exists (left as-is)";
...
export function runInit(root) {
  ...
  console.log(`  - ${ci}`);
  console.log(
    `  - pinned ${Object.keys(pins.files).length} surface files + ${Object.keys(pins.packageScripts).length} package scripts -> ${PINS_PATH}`
  );
  console.log("\nNext:");
  console.log(
    "  1. commit security/surface-pins.json (+ the hook / workflow)."
  );
  console.log(
    "  2. add the 'surface-guard' CI check to your branch-protection required checks,"
  );
  console.log(
    "     and code-own security/ + scripts/ (see .github/CODEOWNERS)."
  );
  console.log(
    "  3. re-pin after any legit surface change: node scripts/guard-surfaces.mjs --update"
  );
}
...
function runGlobalCli(args) {
  ...
  if (args.includes("--update")) {
    ...
    console.log(
      `OK global baseline written for ${Object.keys(snap).length} agent config(s) -> ~/${GLOBAL_BASELINE}`
    );
    return;
  }
  ...
  if (res.needsBaseline) {
    console.log(
      "No global baseline yet. Review your global agent configs, then run:\n  node scripts/guard-surfaces.mjs --global --update"
    );
    return;
  }
...
function main() {
  ...
  if (args.includes("--update")) {
    const pins = updatePins(root);
    console.log(
      `OK re-pinned ${Object.keys(pins.files).length} surface files + ${Object.keys(pins.packageScripts).length} package scripts -> ${PINS_PATH}`
    );
    return;
  }
